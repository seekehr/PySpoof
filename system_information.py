...
from utils.formats import ERROR, INFO
import machineid
import re
...
class SystemInformation:
    ...
    def get(self):
        ...
        try:
            ...
            if self.updated_installdate:
                try:
                    timeStampToDate = datetime.datetime.strptime(self.updated_installdate, '%Y%m%d%H%M%S')
                    output_str += Fore.GREEN + "Windows installed: " + Fore.RESET + str(
                        timeStampToDate) + " || " + self.updated_installdate + "\n"
                except ValueError:
                    output_str += Fore.GREEN + "Windows installed: " + Fore.RESET + f"Invalid Date Format ({self.updated_installdate})" + "\n"
            else:
                output_str += Fore.GREEN + "Windows installed: " + Fore.RESET + "Invalid Install Date" + "\n"
            output_str += Fore.GREEN + "OS Serial: " + Fore.RESET + (self.updated_osserial or "Invalid OS Serial") + "\n"
            output_str += Fore.GREEN + "User SID: " + Fore.RESET + (self.updated_user_sid or "Invalid User SID") + "\n"

            # Network Section
            output_str += Fore.YELLOW + "==============" + Fore.LIGHTMAGENTA_EX + "NET" + Fore.YELLOW + "==============" + Fore.RESET + "\n"
            output_str += Fore.GREEN + "General MAC: " + Fore.RESET + (self.updated_mac or "Invalid MAC") + "\n"
            output_str += Fore.GREEN + "Local IP: " + Fore.RESET + (self.updated_local_ip or "Invalid Local IP") + "\n"
            output_str += Fore.GREEN + "Public IP: " + Fore.RESET + (self.updated_public_ip or "Invalid Public IP") + "\n"
            output_str += Fore.GREEN + "WLAN GUID: " + Fore.RESET + (self.updated_wlan_guid or "N/A or Not Found") + "\n"
            output_str += Fore.GREEN + "WLAN Phys Addr: " + Fore.RESET + (self.updated_wlan_physical_address or "N/A or Not Found") + "\n"
            output_str += Fore.GREEN + "WLAN BSSID: " + Fore.RESET + (self.updated_wlan_bssid or "N/A or Not Connected") + "\n"

            # Disk Section
            output_str += Fore.YELLOW + "==============" + Fore.LIGHTMAGENTA_EX + "DISK" + Fore.YELLOW + "==============" + Fore.RESET + "\n"
            output_str += Fore.GREEN + "Disk Serial: " + Fore.RESET + (self.updated_disk_serial or "Invalid Serial") + "\n"
    ...
    @property
    def updated_mac(self):
        return self._mac()

    @property
    def updated_motherboard(self):
        return self._motherboard()

    @property
    def updated_bios(self):
        return self._bios()
    ...
    def updated_wlan_physical_address(self):
        return self._wlan_info()['physical_address']
    ...
    def _mac(self):
        """Get MAC address of current network adapter (WiFi or Ethernet) using WMI"""
        try:
    ...
    def _computersystem_length(self):
        ...
        try:
            ...
            if not output_decoded:
                self.logger.error(f"Could not decode WMIC output")
                return None
