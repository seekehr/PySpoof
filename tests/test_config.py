...
logger = Logger("resources/tests/test_logs.txt")
config = Config(logger, 'resources/tests/test_config.json')
...
def test_save():
    data = config.getData()
    config.save()
    newConfig = Config(logger, "resources/tests/test_config.json")
    assert newConfig.getData() == data

def test_instance():
    config2 = Config(logger, "resources/tests/test_config.json")
